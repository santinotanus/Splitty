...
export async function updateUser(firebase_uid: string, updates: { nombre?: string; clave_pago?: string | null; foto_url?: string | null }) {
  ...
  // Solo agregar campos que están definidos (undefined significa "no actualizar")
  ...
  console.log('🔧 users.repo.updateUser - firebase_uid:', firebase_uid);
  console.log('🔧 users.repo.updateUser - payload:', payload);

  const result = await db('dbo.usuarios')
    .where({ firebase_uid })
    .update(payload);

  console.log('🔧 users.repo.updateUser - rows affected:', result);
